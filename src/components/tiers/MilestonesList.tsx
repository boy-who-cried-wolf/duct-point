...
const MilestonesList = ({
  ...
  return <Card className="overflow-hidden shadow-none border-none">
      ...
      <CardContent>
        <div className="space-y-3">
          ...
          return <div key={milestone.id} className={`p-3 rounded-md border ${canRedeem ? 'border-primary/30 bg-primary/5' : redeemed ? 'border-green-500/30 bg-green-500/5' : 'border-border'}`}>
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-medium flex items-center gap-1">
                      {milestone.name}
                      {redeemed && (status === 'pending' ? <Clock className="h-3.5 w-3.5 text-amber-500 ml-1" /> : <Check className="h-3.5 w-3.5 text-green-500 ml-1" />)}
                    </h3>
                ...
                <div className="flex justify-between items-center">
                  ...
                  {redeemed ? <Badge variant="outline" className={status === 'pending' ? 'text-amber-500' : 'text-green-500'}>
                      {status === 'pending' ? 'Processing' : 'Redeemed'}
                    </Badge> : <Button size="sm" disabled={!canRedeem} onClick={() => handleRedeem(milestone.id)}>
                      Redeem
          ...
          {milestones.length === 0 && <div className="text-center py-6 text-muted-foreground">
