...
export const checkPlatformRole = async (requiredRole: 'super_admin' | 'staff' | 'user'): Promise<boolean> => {
  try {
    ...
    // First, try to call the dedicated function
    const { data: isRoleAdmin, error: rpcError } = await supabase.rpc('has_platform_role_safe', {
      required_role: requiredRole
    });
    
    if (!rpcError) {
      return isRoleAdmin === true;
    }
    
    logWarning('ROLE CHECK: Using fallback for role check due to RPC error', { error: rpcError });
    
    // Fallback: Call the appropriate function based on the required role
    if (requiredRole === 'super_admin') {
      const { data, error } = await supabase.rpc('is_super_admin');
      if (error) {
        logError('ROLE CHECK: Error checking super_admin role', error);
        return false;
      }
      return data === true;
    }
    ...
    if (requiredRole === 'staff') {
      const { data, error } = await supabase.rpc('is_staff_or_super_admin');
      if (error) {
        logError('ROLE CHECK: Error checking staff role', error);
        return false;
      }
      return data === true;
    }
    
    // For 'user' role, just check if the user is authenticated
    return true;
  } catch (err) {
